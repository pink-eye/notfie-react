...
const DragNDrop: FC = () => {
	...
	const resultImport = (className: string, text: string) => {
		let field = document.querySelector(`.${styles.dragNDrop}`)
	...
	const handleFile = (event: Event) => {
		const dragNDrop = event.target as HTMLInputElement
		const inputFile = dragNDrop.files[0]

		let field = dragNDrop.closest(`.${styles.dragNDrop}`)

		field.classList.remove(`${styles.valid}`)
		field.classList.remove(`${styles.invalid}`)

		changeTip(`I've got a '${inputFile.name}'. You can press 'Import' now`)
